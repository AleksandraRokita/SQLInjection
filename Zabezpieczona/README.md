...
## 5. Zabezpieczenie aplikacji (Rozwiązanie)

Wszystkie powyższe podatności SQL Injection zostały wyeliminowane poprzez zastosowanie **Parametryzacji poleceń (Parameterized Queries)**. 

### Dlaczego to działa?
Zamiast budować zapytania SQL poprzez niebezpieczne sklejanie ciągów znaków z danymi podanymi przez użytkownika (konkatenacja), wykorzystano mechanizm parametrów udostępniany przez bibliotekę `sqlite3`. 

W zapytaniach parametryzowanych silnik bazy danych traktuje wstrzyknięte wartości ściśle jako dane (tekst lub liczby), a nie jako instrukcje do wykonania. Nawet jeśli haker wprowadzi złośliwy kod (np. apostrofy, słowa kluczowe `UNION` czy `--`), zostanie on zinterpretowany wyłącznie jako zwykły tekst, co zapobiega modyfikacji struktury zapytania.
